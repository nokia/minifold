...
def test_cast_none():
    for none in ["none", "None", "NONE", None]:
        assert cast_none(none) == None, "cast_none(%r) should return None"
    for dummy in [True, False, 0, 0.0, "dummy", 1, 1.2]:
        try:
            _ = cast_none(dummy)
            assert False, "cast_none(%r) should raise ValueError" % none
        except ValueError:
            pass

def test_cast_bool():
    for true in ["true", "True", "TRUE"]:
        assert cast_bool(true) == True, "cast_bool(%r) should return True"
    for false in ["false", "False", "FALSE"]:
        assert cast_bool(false) == False, "cast_bool(%r) should return False"
    for dummy in [None, 0, 0.0, "dummy", 1, 1.2]:
        try:
            _ = cast_bool(dummy)
            assert False, "cast_bool(%r) should raise ValueError" % none
        except ValueError:
            pass

def test_lexical_cast():
    assert lexical_cast("None", cast_none) == None
    assert lexical_cast("True", cast_bool) == True
    assert lexical_cast("False", cast_bool) == False
    assert lexical_cast("17", int) == 17
    try:
        lexical_cast("17.2", int) == 17
        assert False, "lexical_cast(%r, int) should raise ValueError" % none
    except ValueError:
        pass
...
def test_lexical_casts():
    ...
    assert lexical_casts("None", cast_operators) == None
    assert lexical_casts("True", cast_operators) == True
